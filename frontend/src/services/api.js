...
api.interceptors.response.use(
  ...
  (error) => {
    if (error.response && error.response.status === 401) {
      ...
      if (msg.includes('revoked') || msg.includes('removed') || msg.includes('logged out')) {
...
export const authAPI = {
  register: async (userId, publicKey, registrationCode) => {
    const response = await api.post('/auth/register', { userId, publicKey, registrationCode });
  ...
  verify: async (userId, proof, trustDevice) => {
    const response = await api.post('/auth/verify', { userId, proof, trustDevice });
    return response.data;
  },
  getMe: async () => {
  ...
  verify: async (userId, proof, trustDevice, deviceId) => {
    const response = await api.post('/auth/verify', { userId, proof, trustDevice, deviceId });
    return response.data;
  },
  //Secure Device Pairing endpoints
  requestPairingCode: async (userId, tempPublicKeyBase64) => { // Updated to send temp key
    const response = await api.post('/auth/pairing/request', { userId, tempPublicKeyBase64 });
    return response.data;
  },
  getTempPublicKey: async (pairingCode) => { // NEW: No-Camera Fallback
    const response = await api.post('/auth/pairing/key', { pairingCode });
    return response.data;
  },
  submitEncryptedKey: async (pairingCode, encryptedPayload) => {
    const response = await api.post('/auth/pairing/submit', { pairingCode, encryptedPayload });
    return response.data;
  },
  pollEncryptedKey: async (userId, pairingCode) => {
    const response = await api.post('/auth/pairing/poll', { userId, pairingCode });
    return response.data;
  },
};
...
export const userAPI = {
  ...
  assignPanel: async (studentId, panelIds, startDate, endDate) => {
    ...
    const response = await api.post('/timetables/assign-panel', {
      studentId, panelIds, startDate, endDate
    });
    return response.data;
  },
...
export const evaluationAPI = {
  ...
  getStats: async (studentId) => {
    const response = await api.get(`/evaluations/stats${studentId ? `?studentId=${studentId}` : ''}`);
    return response.data;
  },
...
export const timetableAPI = {
  ...
  uploadDocument: async (sessionId, documentData) => {
    const response = await api.post(`/timetables/${sessionId}/documents`, documentData);
    return response.data;
  },
  deleteDocument: async (sessionId, documentId) => {
    const response = await api.delete(`/timetables/${sessionId}/documents/${documentId}`);
    return response.data;
  },
  addNotes: async (sessionId, notesData) => {
    const response = await api.post(`/timetables/${sessionId}/notes`, notesData);
    return response.data;
  },
};
...
export const feedbackAPI = {
  ...
  getStats: async (studentId) => {
    const response = await api.get(`/feedback/stats${studentId ? `?studentId=${studentId}` : ''}`);
    return response.data;
  },
...
export const attendanceAPI = {
  ...
  getStats: async (studentId) => {
    const response = await api.get(`/attendance/stats${studentId ? `?studentId=${studentId}` : ''}`);
    return response.data;
  },
