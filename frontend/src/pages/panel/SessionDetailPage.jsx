...
export default function SessionDetailPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start mb-4">
          <div>
            ...
            <span
              className={`inline-block px-3 py-1 rounded-full text-sm font-semibold ${getStatusColor(session.status)}`}
            >
              {session.status}
            </span>
          </div>
          {["panel", "coordinator"].includes(user?.role) && (
            <button
              onClick={() =>
                navigate(
                  `/panel/evaluation/new?student=${session.students?.[0]?._id}`,
                )
              }
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm mt-4">
          ...
          <div className="flex items-center gap-2">
            <Clock className="w-5 h-5 text-gray-400" /> {session.startTime} -{" "}
            {session.endTime}
          </div>
          <div className="flex items-center gap-2">
            <MapPin className="w-5 h-5 text-gray-400" /> {session.venue}
          </div>
          <div className="flex items-center gap-2">
            <Users className="w-5 h-5 text-gray-400" />{" "}
            {session.students?.[0]?.name || "No Student"}
          </div>
        </div>
      </div>

      {/* 2. Command Center (STRICTLY FOR PANELS ONLY) */}
      {user?.role === "panel" && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {!attendanceActive ? (
              <div className="text-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                <p className="text-gray-600 mb-4 text-sm">
                  Open the gateway to allow students and co-panels to sign in.
                </p>
                <button
              ...
              <div className="flex flex-col items-center space-y-4 bg-indigo-50 p-6 rounded-lg">
                ...
                <div className="text-center w-full">
                  ...
                  <p className="text-4xl font-black text-indigo-600 tracking-[0.25em]">
                    {attendanceCode}
                  </p>
                </div>
                <button
          ...
          {session.panels && session.panels.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h2 className="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                <Users className="w-5 h-5 text-indigo-600" /> Co-Panel
                Verification
              </h2>
                  ...
                  .map((panel) => (
                    ...
                    >
                      ...
                      <div className="flex flex-wrap gap-2 items-center">
                        <button
                          onClick={() =>
                            markCoPanelAttendance(panel._id, "present")
                          }
                          className="px-3 py-1.5 bg-green-600 text-white text-xs font-bold rounded"
                        >
                          Confirm Present
                        </button>
                        <input
                          ...
                          className="text-xs px-2 py-1.5 border rounded w-32"
                          onChange={(e) => setAbsenceReason(e.target.value)}
                        />
                        <button
                          onClick={() =>
                            markCoPanelAttendance(panel._id, "absent")
                          }
                          className="px-3 py-1.5 bg-red-100 text-red-700 text-xs font-bold rounded border border-red-200"
                        >
                          Mark Absent
                        </button>
                      </div>
      ...
      <div className="mb-6">
        <SessionDocuments session={session} onUpdate={loadSession} />
      </div>
      {["panel", "coordinator"].includes(user?.role) && (
        <div className="mb-6">
          <PanelPreReviewNotes session={session} onUpdate={loadSession} />
        </div>
      )}
    </div>
  );
