...
import { Users, UserPlus, Shield, X, Copy, CheckCircle, Search } from 'lucide-react';
...
export default function UsersPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    matricNumber: '', program: '', researchTitle: '', supervisorId: ''
  });
  ...
  const supervisors = usersList.filter(u => u.role === 'supervisor' || u.role === 'panel');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        navigator.clipboard.writeText(`Welcome ${res.data.user.name}!\nYour System ID: ${res.data.user.userId}\nYour First-Time Setup Code: ${res.data.registrationCode}`);
        
        setShowModal(false);
        setFormData({ userId: '', name: '', email: '', role: '', matricNumber: '', program: '', researchTitle: '', supervisorId: '' });
        fetchUsers();
      }
    } catch (error) {
  ...
  const getRoleBadge = (role) => {
    ...
    return <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${styles[role] || 'bg-gray-100'}`}>{role}</span>;
  };
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 mt-2">
            {isSuperAdmin ? 'Full system control. Manage Admins and higher.' : 'Manage Students, Panels, and Supervisors.'}
          </p>
        </div>
        <button
      ...
      {newCredentials && (
        <div className="bg-green-50 border-2 border-green-500 rounded-xl p-6 shadow-lg mb-6 relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4">
            <button onClick={() => setNewCredentials(null)} className="text-green-700 hover:text-green-900">
              <X className="w-6 h-6" />
            </button>
          </div>
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-xl font-bold text-green-900 mb-1">User Created Successfully!</h3>
              <p className="text-green-800 mb-4">Please copy these credentials and send them to <strong>{newCredentials.name}</strong> securely. They will need this code for their first login.</p>
              
              <div className="bg-white p-4 rounded-lg border border-green-200 inline-block">
                <p className="text-sm text-gray-600 mb-1">User ID:</p>
                <p className="font-mono text-lg font-bold text-gray-900 mb-3">{newCredentials.userId}</p>
                
                <p className="text-sm text-gray-600 mb-1">First-Time Registration Code:</p>
                <div className="flex items-center gap-3">
                  ...
                  <button 
                    onClick={() => {
                      navigator.clipboard.writeText(`User ID: ${newCredentials.userId}\nRegistration Code: ${newCredentials.registrationCode}`);
                    ...
                    {u.role === 'student' ? (
                      u.supervisorId ? (
                        <div className="flex flex-col">
                          <span className="font-semibold text-gray-800">{u.supervisorId.name}</span>
                          <span className="text-xs text-indigo-600 font-medium">Supervisor</span>
                        </div>
                      ) : (
                        <span className="text-red-500 font-medium">No Supervisor Assigned</span>
                      )
                    ) : (
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-white z-10">
              <h2 className="text-xl font-bold text-gray-900">Create New System User</h2>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="grid grid-cols-2 gap-5">
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">System User ID *</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-2 gap-5">
                <div>
                  ...
                  <input type="text" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full border border-gray-300 rounded p-2" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1">Email Address *</label>
                  <input type="email" required value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} className="w-full border border-gray-300 rounded p-2" />
                </div>
              </div>
              ...
              {formData.role === 'student' && (
                <div className="bg-indigo-50 p-5 rounded-lg border border-indigo-100 space-y-4 mt-4">
                  <h4 className="font-bold text-indigo-900 border-b border-indigo-200 pb-2">Academic Details</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">Matric Number *</label>
                      <input type="text" required value={formData.matricNumber} onChange={e => setFormData({...formData, matricNumber: e.target.value})} className="w-full border border-gray-300 rounded p-2" />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">Program *</label>
                      <select required value={formData.program} onChange={e => setFormData({...formData, program: e.target.value})} className="w-full border border-gray-300 rounded p-2">
                  ...
                  <div>
                    ...
                    <input type="text" value={formData.researchTitle} onChange={e => setFormData({...formData, researchTitle: e.target.value})} className="w-full border border-gray-300 rounded p-2" />
                  </div>
                  <div>
                    ...
                    >
                      ...
                      {supervisors.map(sup => (
                        <option key={sup._id} value={sup._id}>{sup.name} ({sup.userId})</option>
                      ))}
                    </select>
                  </div>
                </div>
              ...
              <div className="pt-4 flex justify-end gap-3 border-t border-gray-100">
                ...
                <button type="submit" className="px-6 py-2 bg-indigo-600 text-white font-bold rounded shadow hover:bg-indigo-700">Create User & Generate Code</button>
              </div>
            </form>
          </div>
        </div>
