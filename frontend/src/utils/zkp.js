...
class ZKPAuth {
  ...
  async storePrivateKey(userId, privateKeyHex) {
    localStorage.setItem(`${ZKP_CONFIG.storageKeys.privateKey}_${userId}`, privateKeyHex);
  }
  ...
  hasKeysForUser(userId) {
    return !!localStorage.getItem(`${ZKP_CONFIG.storageKeys.privateKey}_${userId}`);
  }
  ...
  async generateTempSyncKeyPair() {
    ...
    return {
      ...
      tempPublicKeyBase64: btoa(String.fromCharCode(...new Uint8Array(exportedPub)))
    };
  }
  ...
  async encryptPayload(publicKeyBase64, payloadString) {
    ...
    const rsaPubKey = await window.crypto.subtle.importKey("spki", binaryDer, { name: "RSA-OAEP", hash: "SHA-256" }, true, ["encrypt"]);
  ...
  async decryptSyncKey(encryptedPackageString, privateKey) {
    ...
    const encryptedAesKeyBuffer = Uint8Array.from(atob(pkg.encryptedAesKey), c => c.charCodeAt(0));
    ...
    const payloadBuffer = Uint8Array.from(atob(pkg.payload), c => c.charCodeAt(0));
    const decryptedPayload = await window.crypto.subtle.decrypt({ name: "AES-GCM", iv }, aesKey, payloadBuffer);
