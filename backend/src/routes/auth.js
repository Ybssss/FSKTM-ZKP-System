...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        message: `${targetUser.name} logged out from all devices`,
      });
    } catch (error) {
      res
        .status(500)
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res.json({
        success: true,
        devices: targetUser.authenticatedDevices || [],
      });
    } catch (error) {
      res
        .status(500)
        .json({ success: false, message: "Failed to get user devices" });
    }
  },
);
