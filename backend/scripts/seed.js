...
const seedDatabase = async () => {
  try {
    ...
    const demoPanels = [
      {
        ...
        role: "panel",
        profession: "Dr. · Jabatan Keselamatan Maklumat dan Teknologi Web",
      ...
      },
      {
        ...
        role: "panel",
        profession: "Dr. · Jabatan Kejuruteraan Perisian",
      ...
      },
      {
        ...
        role: "panel",
        profession: "Prof. Madya Dr. · Jabatan Keselamatan Maklumat dan Teknologi Web",
      ...
      },
      {
        ...
        role: "panel",
        profession: "Ts. Dr. · Jabatan Keselamatan Maklumat dan Teknologi Web",
    ...
    // Fixed demo dates for clearer UAT: June to August.
    const lastMonth = new Date("2026-06-10T00:00:00.000Z");
    const lastWeek = new Date("2026-07-08T00:00:00.000Z");
    const yesterday = new Date("2026-07-22T00:00:00.000Z");
    const tomorrow = new Date("2026-08-03T00:00:00.000Z");
    const nextWeek = new Date("2026-08-07T00:00:00.000Z");
    const inThreeDays = new Date("2026-08-10T00:00:00.000Z");
    const inTwoWeeks = new Date("2026-08-17T00:00:00.000Z");
    const inThreeWeeks = new Date("2026-08-24T00:00:00.000Z");
    const inOneMonth = new Date("2026-08-31T00:00:00.000Z");
    ...
    await SessionBatch.create([
      ...
      {
        batchName: "FSKTM Completed History Batch",
        batchId: "BATCH-PRS-2026-HISTORY",
        academicSession: "2025/2026, Semester 1",
        scheduleTitle: "Postgraduate Progress Presentation Schedule",
        sessionType: "PROGRESS_ASSESSMENT",
        ...
        breakBetweenSlotsMinutes: 5,
        googleMeetLink: "https://meet.google.com/fsktm-completed-history",
        status: "completed",
        createdBy: getUserId("samihah@uthm.edu.my"),
      },
      {
    ...
    const completedHistoryPlan = batchSessionPlan.map((slot, index) => {
      const sessionTypes = [
        "PROPOSAL_DEFENSE",
        "PROGRESS_ASSESSMENT",
        "PRE_VIVA",
      ...
      const rubricByType = {
        PROPOSAL_DEFENSE: proposalRubric._id,
        PROGRESS_ASSESSMENT: progressRubric._id,
        PRE_VIVA: preVivaRubric._id,
      };

      const startTime = addMinutesToHHMM("09:00", index * 35);

      return {
        ...slot,
        sessionType: type,
        rubricId: rubricByType[type],
        date: datePool[index],
        startTime,
        endTime: addMinutesToHHMM(startTime, 30),
      };
    });
    ...
    sessionsData.push(
      ...completedHistoryPlan.map((slot) => {
        ...
        const sessionTypeLabel =
          slot.sessionType === "PROPOSAL_DEFENSE"
            ? "Proposal Defense"
            : slot.sessionType === "PRE_VIVA"
              ? "Pre-Viva"
              : "Progress Assessment";
        ...
        return {
          students: [getUserId(slot.studentEmail)],
          panels: slot.panels,
          title: `Completed ${sessionTypeLabel} - ${student.name}`,
          sessionType: slot.sessionType,
          rubricId: slot.rubricId,
          date: slot.date,
          startTime: slot.startTime,
          endTime: slot.endTime,
          venue: "https://meet.google.com/fsktm-completed-history",
          googleMeetLink: "https://meet.google.com/fsktm-completed-history",
          status: "completed",
          batchId: "BATCH-PRS-2026-HISTORY",
          batchName: "FSKTM Completed History Batch",
          ...
          studentDocuments: [
            {
              title: `${student.name} ${sessionTypeLabel} Report`,
              url: "https://drive.google.com/file/d/demo-completed-report",
              source: "external-link",
              type: "report",
              uploadedBy: getUserId(slot.studentEmail),
              description:
                "Seeded student-uploaded report for completed historical evaluation.",
            },
            {
              title: `${student.name} ${sessionTypeLabel} Slides`,
              url: "https://drive.google.com/file/d/demo-completed-slides",
              source: "external-link",
              type: "slides",
              uploadedBy: getUserId(slot.studentEmail),
              description:
                "Seeded student-uploaded slides for completed historical evaluation.",
            },
          ],
        };
      }),
    ...
    for (const slot of completedHistoryPlan) {
      ...
      for (const [panelIndex, panelId] of slot.panels.entries()) {
        const completedEvaluation = {
          sessionId: getSessionIdByTitle(
            `Completed ${sessionTypeLabel} - ${student.name}`,
          ),
          rubricId: slot.rubricId,
          studentId: getUserId(slot.studentEmail),
          evaluatorId: panelId,
        ...
        if (slot.sessionType === "PROGRESS_ASSESSMENT") {
          Object.assign(completedEvaluation, {
            summaryOfProgress:
              "The student has completed the planned milestone and presented research progress with supporting materials.",
            commentsForImprovement:
              "Improve technical validation, strengthen literature linkage, and prepare clearer discussion of limitations.",
            overallSuggestions:
              panelIndex === 0
                ? "Proceed to the next phase with minor improvements."
                : "Continue implementation and prepare stronger evidence for the next review.",
          });
        } else if (slot.sessionType === "PROPOSAL_DEFENSE") {
          Object.assign(completedEvaluation, {
            totalMarks: panelIndex === 0 ? 76 : 82,
            scores: makeScoreMapFromValues(proposalRubric, {}),
            qualitativeFeedback: makeQualitativeMap(
              proposalRubric,
              "The proposal is acceptable. The candidate should refine methodology justification and strengthen expected contribution.",
            ),
            overallComments:
              panelIndex === 0
                ? "Good proposal direction with minor amendments required."
                : "Clear research motivation and acceptable scope.",
          });
        } else {
          Object.assign(completedEvaluation, {
            totalMarks: panelIndex === 0 ? 78.5 : 83,
            scores: makeScoreMapFromValues(preVivaRubric, {}),
            qualitativeFeedback: makeQualitativeMap(
              preVivaRubric,
              "The thesis is generally ready for the next stage. Improve presentation of findings and contribution statement.",
            ),
            overallComments:
              panelIndex === 0
                ? "Pass with minor amendments."
                : "Good research maturity and defence readiness.",
          });
        }

        evaluationsData.push(completedEvaluation);
      }
    }
    ...
    const studentsWithCompletedEvaluations = await Evaluation.distinct("studentId", {
      status: "COMPLETED",
    });
    console.log(
