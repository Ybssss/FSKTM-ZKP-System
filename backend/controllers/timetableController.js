...
exports.updateTimetable = async (req, res) => {
  try {
    if (req.user.role !== "admin") {
      return res.status(403).json({
        success: false,
        message: "Only admins can update timetable sessions.",
      });
    }
    ...
    if (req.body.title !== undefined) updates.title = cleanText(req.body.title, 150);
    if (req.body.description !== undefined) updates.description = cleanText(req.body.description, 1000);
    if (req.body.date !== undefined) updates.date = normalizeDateOnly(req.body.date);
    if (req.body.startTime !== undefined) updates.startTime = cleanText(req.body.startTime, 20);
    if (req.body.time !== undefined) updates.startTime = cleanText(req.body.time, 20);
    ...
    if (req.body.rubricId !== undefined) updates.rubricId = req.body.rubricId || null;
    ...
    if (req.body.batchName !== undefined) updates.batchName = cleanText(req.body.batchName, 100);
    if (req.body.batchId !== undefined) updates.batchId = cleanText(req.body.batchId, 150);
    if (req.body.academicSession !== undefined) updates.academicSession = cleanText(req.body.academicSession, 100);
    if (req.body.scheduleTitle !== undefined) updates.scheduleTitle = cleanText(req.body.scheduleTitle, 150);
    ...
    const oldPanelIds = (existingSession.panels || []).map((panelId) => String(panelId));
    const requestedPanels = cleanArray(req.body.panels).length
      ? cleanArray(req.body.panels)
      : [
          req.body.panel1Id !== undefined ? req.body.panel1Id : oldPanelIds[0],
          req.body.panel2Id !== undefined ? req.body.panel2Id : oldPanelIds[1],
        ];

    const newPanelIds = requestedPanels
      .filter(Boolean)
      .map((panelId) => String(panelId));

    const panelsChanged =
      newPanelIds.length > 0 &&
      (newPanelIds.length !== oldPanelIds.length ||
        newPanelIds.some((panelId, index) => panelId !== oldPanelIds[index]));

    if (panelsChanged) {
      if (new Set(newPanelIds).size !== newPanelIds.length) {
        return res.status(400).json({
          success: false,
    ...
    const pendingEvaluationBaseUpdate = {};
    if (updates.rubricId !== undefined) pendingEvaluationBaseUpdate.rubricId = updates.rubricId;
    if (updates.sessionType) pendingEvaluationBaseUpdate.sessionType = updates.sessionType;

    if (panelsChanged) {
      // Existing completed evaluations are academic records and are never changed.
      // Pending evaluations are assignment placeholders, so recreate them to match the updated session panels exactly.
      await Evaluation.deleteMany({ sessionId: id, status: "PENDING" });

      const studentId = existingSession.students?.[0];
      if (studentId && newPanelIds.length > 0) {
        const evaluationsToInsert = newPanelIds.map((evaluatorId) => ({
          sessionId: id,
          studentId,
          evaluatorId,
          rubricId: updates.rubricId || existingSession.rubricId,
          semester: req.body.semester || existingSession.academicSession || "",
          sessionType: updates.sessionType || existingSession.sessionType,
          status: "PENDING",
        }));

        await Evaluation.insertMany(evaluationsToInsert);
      }
    } else if (Object.keys(pendingEvaluationBaseUpdate).length > 0) {
      await Evaluation.updateMany(
        { sessionId: id, status: "PENDING" },
        pendingEvaluationBaseUpdate,
      );
    }

    res.json({
      success: true,
      message: panelsChanged
        ? "Session updated. Pending evaluator assignments were refreshed. Completed evaluations were not changed."
        : "Session updated successfully.",
      timetable: formatTimetable(updatedSession),
    });
  } catch (error) {
    res.status(500).json({
      success: false,
